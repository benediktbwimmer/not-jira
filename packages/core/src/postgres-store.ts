...
export class PostgresStore implements AppStore {
  ...
  constructor(
    ...
    this.projects = new PostgresProjectRepository(
      this.queryable,
      this.tenantId,
    );
    this.tasks = new PostgresTaskRepository(this.queryable, this.tenantId);
    this.dependencies = new PostgresDependencyRepository(
      this.queryable,
      this.tenantId,
    );
    this.comments = new PostgresCommentRepository(
      this.queryable,
      this.tenantId,
    );
    ...
    this.instructions = new PostgresInstructionRepository(
      this.queryable,
      this.tenantId,
    );
    ...
    this.activity = new PostgresActivityRepository(
      this.queryable,
      this.tenantId,
    );
    this.migrations = new PostgresMigrationRepository(this.queryable);
    this.outbox = new PostgresOutboxEventRepository(
      this.queryable,
      this.tenantId,
    );
    this.inbox = new PostgresInboxEventRepository(
      this.queryable,
      this.tenantId,
    );
    this.matcher = new PostgresMatcherRepository(this.queryable, this.tenantId);
    this.hostedIdentity = new PostgresHostedIdentityRepository(
      this.queryable,
      this.tenantId,
    );
    this.hostedAudit = new PostgresHostedAuditRepository(
      this.queryable,
      this.tenantId,
    );
    ...
    this.connectors = new PostgresConnectorRepository(
      this.queryable,
      this.tenantId,
    );
  }
...
class PostgresMatcherRepository implements MatcherQueryRepository {
  ...
  async matchTaskIds(
    ...
    const result = await this.db.query(lowered.taskIds.sql, [
      this.tenantId,
      ...lowered.taskIds.params,
    ]);
...
class PostgresHostedAuditRepository implements HostedAuditRepository {
  ...
  async list(
    ...
    const result = options.projectId === undefined
      ...
      : await this.db.query(
        "select * from hosted_audit_events where tenant_id = $1 and project_id is not distinct from $2 order by created_at desc limit $3",
        [tenantId, options.projectId, limit],
      );
...
class PostgresHostedSecretRepository implements HostedSecretRepository {
  ...
  async get(id: string): Promise<HostedSecret | null> {
    const result = await this.db.query(
      "select * from hosted_secrets where tenant_id = $1 and id = $2",
      [this.tenantId, id],
    );
  ...
  async list(projectId?: string | null): Promise<HostedSecret[]> {
    const result = projectId === undefined
      ? await this.db.query(
        "select * from hosted_secrets where tenant_id = $1 order by updated_at desc, name asc",
        [this.tenantId],
      )
      : await this.db.query(
        "select * from hosted_secrets where tenant_id = $1 and project_id is not distinct from $2 order by updated_at desc, name asc",
...
class PostgresConnectorRepository implements ConnectorRepository {
  ...
  async getConnection(
    ...
    const result = await this.db.query(
      "select * from connector_connections where tenant_id = $1 and project_id = $2 and id = $3",
  ...
  async listConnections(projectId?: string): Promise<ConnectorConnection[]> {
    const result = projectId
      ? await this.db.query(
        "select * from connector_connections where tenant_id = $1 and project_id = $2 order by updated_at desc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from connector_connections where tenant_id = $1 order by project_id asc, updated_at desc, id asc",
        [this.tenantId],
      );
  ...
  async getMappingByExternal(
    ...
    return result.rows[0]
      ? connectorExternalMappingFromRow(result.rows[0])
      : null;
  }
...
export async function createPostgresStore(
  ...
  const connectionString = options.connectionString ??
    process.env.UNBLOCK_POSTGRES_URL;
  if (!options.pool && !connectionString) {
    validation(
      "Postgres storage requires UNBLOCK_POSTGRES_URL or PostgresStoreOptions.connectionString.",
    );
  }
...
class PostgresProjectRepository implements ProjectRepository {
  ...
  async list(): Promise<Project[]> {
    const result = await this.db.query(
      "select * from projects where tenant_id = $1 order by name asc, id asc",
      [this.tenantId],
    );
...
class PostgresTaskRepository implements TaskRepository {
  ...
  async list(projectId?: string): Promise<Task[]> {
    const result = projectId
      ? await this.db.query(
        "select * from tasks where tenant_id = $1 and project_id = $2 order by created_at asc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from tasks where tenant_id = $1 order by project_id asc, created_at asc, id asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<Task | null> {
    const result = await this.db.query(
      "select * from tasks where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
  ...
  async updateWithPrevious(previous: Task, task: Task): Promise<void> {
    ...
    if (result.rowCount !== 1) {
      validation("Task version conflict.", {
        taskId: task.id,
  ...
  async delete(projectId: string, id: string): Promise<void> {
    await this.db.query(
      "update tasks set parent_task_id = null where tenant_id = $1 and project_id = $2 and parent_task_id = $3",
      [this.tenantId, projectId, id],
    );
    await this.db.query(
      "delete from tasks where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
  }
}
...
class PostgresDependencyRepository implements DependencyRepository {
  ...
  async list(projectId?: string): Promise<Dependency[]> {
    const result = projectId
      ? await this.db.query(
        "select * from task_dependencies where tenant_id = $1 and project_id = $2 order by task_id, depends_on_task_id",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from task_dependencies where tenant_id = $1 order by project_id, task_id, depends_on_task_id",
        [this.tenantId],
      );
  ...
  async listForTask(projectId: string, taskId: string): Promise<Dependency[]> {
    const result = await this.db.query(
      "select * from task_dependencies where tenant_id = $1 and project_id = $2 and task_id = $3 order by depends_on_task_id",
      [this.tenantId, projectId, taskId],
    );
    return result.rows.map(dependencyFromRow);
  }
  ...
  async listDependents(
    ...
    const result = await this.db.query(
      "select * from task_dependencies where tenant_id = $1 and project_id = $2 and depends_on_task_id = $3 order by task_id",
      [this.tenantId, projectId, dependsOnTaskId],
    );
    return result.rows.map(dependencyFromRow);
  }
  ...
  async inspectAdd(projectId: string, taskId: string, dependsOnTaskId: string) {
    if (isPoolClient(this.db)) {
      ...
      const exists = await this.hasDependency(
        projectId,
        taskId,
        dependsOnTaskId,
      );
      const createsDependencyCycle = await this.hasDependencyPath(
        projectId,
        dependsOnTaskId,
        taskId,
      );
      const taskContainsDependsOnTask = await this.hasHierarchyPath(
        projectId,
        taskId,
        dependsOnTaskId,
      );
      const dependsOnTaskContainsTask = await this.hasHierarchyPath(
        projectId,
        dependsOnTaskId,
        taskId,
      );
      return {
        ...
        taskContainsDependsOnTask,
        dependsOnTaskContainsTask,
      };
    }
    const [
      task,
  ...
  async hasDependency(
    ...
    const result = await this.db.query(
      "select 1 from task_dependencies where tenant_id = $1 and project_id = $2 and task_id = $3 and depends_on_task_id = $4 limit 1",
      [this.tenantId, projectId, taskId, dependsOnTaskId],
    );
    return (result.rowCount ?? 0) > 0;
  }

  async hasDependencyPath(
    projectId: string,
    fromTaskId: string,
    toTaskId: string,
  ): Promise<boolean> {
  ...
  async remove(
    ...
    await this.db.query(
      "delete from task_dependencies where tenant_id = $1 and project_id = $2 and task_id = $3 and depends_on_task_id = $4",
      [this.tenantId, projectId, taskId, dependsOnTaskId],
    );
  }
  ...
  async replaceForTask(
    ...
    await this.db.query(
      "delete from task_dependencies where tenant_id = $1 and project_id = $2 and task_id = $3",
      [this.tenantId, projectId, taskId],
    );
  ...
  private async taskSummary(
    ...
    const result = await this.db.query(
      "select id, archived_at from tasks where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, taskId],
    );
    return result.rows[0]
      ? {
        id: result.rows[0].id,
        archivedAt: nullableIso(result.rows[0].archived_at),
      }
      : null;
  }
}
...
class PostgresCommentRepository implements CommentRepository {
  ...
  async list(projectId?: string): Promise<Comment[]> {
    const result = projectId
      ? await this.db.query(
        "select * from comments where tenant_id = $1 and project_id = $2 order by created_at asc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from comments where tenant_id = $1 order by project_id, created_at asc, id asc",
        [this.tenantId],
      );
  ...
  async listForTask(projectId: string, taskId: string): Promise<Comment[]> {
    const result = await this.db.query(
      "select * from comments where tenant_id = $1 and project_id = $2 and task_id = $3 order by created_at asc, id asc",
      [this.tenantId, projectId, taskId],
    );
    return result.rows.map(commentFromRow);
  }
  ...
  async get(projectId: string, id: string): Promise<Comment | null> {
    const result = await this.db.query(
      "select * from comments where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
...
class PostgresTagRepository implements TagRepository {
  ...
  async list(projectId?: string): Promise<Tag[]> {
    const result = projectId
      ? await this.db.query(
        "select * from tags where tenant_id = $1 and project_id = $2 order by sort_order asc, name asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from tags where tenant_id = $1 order by project_id, sort_order asc, name asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<Tag | null> {
    const result = await this.db.query(
      "select * from tags where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
  ...
  async findByName(projectId: string, name: string): Promise<Tag | null> {
    const result = await this.db.query(
      "select * from tags where tenant_id = $1 and project_id = $2 and name = $3",
      [this.tenantId, projectId, name],
    );
    return result.rows[0] ? tagFromRow(result.rows[0]) : null;
  }
  ...
  async listTaskTags(projectId?: string): Promise<TaskTag[]> {
    const result = projectId
      ? await this.db.query(
        "select * from task_tags where tenant_id = $1 and project_id = $2 order by task_id, tag_id",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from task_tags where tenant_id = $1 order by project_id, task_id, tag_id",
        [this.tenantId],
      );
  ...
  async hasTaskTag(
    ...
    const result = await this.db.query(
      "select 1 from task_tags where tenant_id = $1 and project_id = $2 and task_id = $3 and tag_id = $4 limit 1",
      [this.tenantId, projectId, taskId, tagId],
    );
    return (result.rowCount ?? 0) > 0;
  }
  ...
  async removeTaskTag(
    ...
    await this.db.query(
      "delete from task_tags where tenant_id = $1 and project_id = $2 and task_id = $3 and tag_id = $4",
      [this.tenantId, projectId, taskId, tagId],
    );
  }
}
...
class PostgresTrackRepository implements TrackRepository {
  ...
  async list(projectId?: string): Promise<Track[]> {
    const result = projectId
      ? await this.db.query(
        "select * from tracks where tenant_id = $1 and project_id = $2 order by machine asc, actor asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from tracks where tenant_id = $1 order by project_id, machine asc, actor asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<Track | null> {
    const result = await this.db.query(
      "select * from tracks where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
  ...
  async findByActor(
    ...
    const result = await this.db.query(
      "select * from tracks where tenant_id = $1 and project_id = $2 and machine = $3 and actor = $4",
      [this.tenantId, projectId, machine, actor],
    );
    return result.rows[0] ? trackFromRow(result.rows[0]) : null;
  }
  ...
  async listAssignments(projectId?: string): Promise<TrackAssignment[]> {
    const result = projectId
      ? await this.db.query(
        "select * from track_assignments where tenant_id = $1 and project_id = $2 order by track_id, position",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from track_assignments where tenant_id = $1 order by project_id, track_id, position",
        [this.tenantId],
      );
  ...
  async unassign(
    ...
    await this.db.query(
      "delete from track_assignments where tenant_id = $1 and project_id = $2 and track_id = $3 and task_id = $4",
      [this.tenantId, projectId, trackId, taskId],
    );
  }
...
class PostgresActivityRepository implements ActivityRepository {
  ...
  async list(
    ...
    const result = projectId === null
      ? await this.db.query(
        "select * from activity where tenant_id = $1 order by created_at desc, id desc limit $2",
        [this.tenantId, limit],
      )
      : await this.db.query(
        "select * from activity where tenant_id = $1 and project_id = $2 order by created_at desc, id desc limit $3",
        [this.tenantId, projectId, limit],
      );
  ...
  async version(projectId: string | null = null): Promise<string> {
    const result = projectId === null
      ? await this.db.query(
        "select count(*)::int as count, max(created_at) as max_created_at from activity where tenant_id = $1",
        [this.tenantId],
      )
      : await this.db.query(
        "select count(*)::int as count, max(created_at) as max_created_at from activity where tenant_id = $1 and project_id = $2",
        [this.tenantId, projectId],
      );
    ...
    const maxCreatedAt = row?.max_created_at instanceof Date
...
class PostgresOutboxEventRepository implements OutboxEventRepository {
  ...
  async get(id: string): Promise<OutboxEvent | null> {
    const result = await this.db.query(
      "select * from outbox_events where tenant_id = $1 and id = $2",
      [this.tenantId, id],
    );
  ...
  async findByIdempotencyKey(
    ...
    const result = await this.db.query(
      "select * from outbox_events where tenant_id = $1 and idempotency_key = $2",
...
class PostgresInboxEventRepository implements InboxEventRepository {
  ...
  async get(id: string): Promise<InboxEvent | null> {
    const result = await this.db.query(
      "select * from inbox_events where tenant_id = $1 and id = $2",
      [this.tenantId, id],
    );
  ...
  async findBySource(
    ...
    const result = await this.db.query(
      "select * from inbox_events where tenant_id = $1 and source = $2 and external_event_id = $3",
      [this.tenantId, source, externalEventId],
    );
    return result.rows[0] ? inboxEventFromRow(result.rows[0]) : null;
  }
...
class PostgresInstructionRepository implements InstructionRepository {
  ...
  async list(projectId?: string): Promise<Instruction[]> {
    const result = projectId
      ? await this.db.query(
        "select * from instructions where tenant_id = $1 and project_id = $2 order by name asc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from instructions where tenant_id = $1 order by project_id, name asc, id asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<Instruction | null> {
    const result = await this.db.query(
      "select * from instructions where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
...
class PostgresSavedViewRepository implements SavedViewRepository {
  ...
  async list(projectId?: string): Promise<SavedView[]> {
    const result = projectId
      ? await this.db.query(
        "select * from saved_views where tenant_id = $1 and project_id = $2 order by name asc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from saved_views where tenant_id = $1 order by project_id, name asc, id asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<SavedView | null> {
    const result = await this.db.query(
      "select * from saved_views where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
...
class PostgresQueueFeedRepository implements QueueFeedRepository {
  ...
  async list(projectId?: string): Promise<QueueFeed[]> {
    const result = projectId
      ? await this.db.query(
        "select * from queue_feeds where tenant_id = $1 and project_id = $2 order by name asc, id asc",
        [this.tenantId, projectId],
      )
      : await this.db.query(
        "select * from queue_feeds where tenant_id = $1 order by project_id, name asc, id asc",
        [this.tenantId],
      );
  ...
  async get(projectId: string, id: string): Promise<QueueFeed | null> {
    const result = await this.db.query(
      "select * from queue_feeds where tenant_id = $1 and project_id = $2 and id = $3",
      [this.tenantId, projectId, id],
    );
...
function connectorConnectionParams(
  tenantId: string,
...
function connectorSyncRunParams(
  tenantId: string,
...
function connectorExternalMappingParams(
  tenantId: string,
...
function jsonValuesPlaceholders(
  rowCount: number,
  columnCount: number,
  ...
  return Array.from({ length: rowCount }, (_row, rowIndex) => {
    const columns = Array.from(
      ...
      (_column, columnIndex) => {
        ...
        return jsonColumns.has(columnIndex + 1)
          ? `${placeholder}::jsonb`
          : placeholder;
      },
    );
